...
final class SearchController : BaseViewController<SearchView> {
    ...
    private var searchQueryTimer: Timer?
...
extension SearchController : UISearchBarDelegate {
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        self.rootView.searchStarted(searchText.count > 0)
        if searchText.count > 0 {
            searchQueryTimer?.invalidate()
            let delayInterval: TimeInterval = 0.5
            searchQueryTimer = Timer.scheduledTimer(withTimeInterval: delayInterval, repeats: false) { _ in
